...
def limitted_pattern_spray():
    energies = (5*GeV)/2**np.arange(10)
    for particle in ['e+', 'e-']:
...
def electron_pattern_spray(num):
    energies = (5*GeV)/2**np.arange(10)
    weights = np.ones([10])
    ...
    for i in range(num):
        for particle in ['e+', 'e-']:
            yield particle, g4.G4ThreeVector(), g4.G4ThreeVector(0,0,1), energy[i]

def pattern_spray(num):
    energies = (5*GeV)/2**np.arange(10)
    weights = np.ones([10])
    energy = random.choices(energies, weights, k=num)
    y_lower = -29
    y_upper = 30
    ys = np.linspace(y_lower, y_upper, 10)
    weights = np.ones([10])
    y = random.choices(ys, weights, k=num)
    for i in range(num):
        for particle in ['gamma']:
            yield particle, g4.G4ThreeVector(0, y[i]*mm, -25*mm), g4.G4ThreeVector(0,0,1), energy[i]
